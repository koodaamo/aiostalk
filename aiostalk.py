...
class Client:
    """A client implementing the beanstalk protocol.
    Upon creation a connection with beanstalkd is established and tubes
    are initialized.
    ...
    :param watch: The tubes to watch after connecting. The ``default``
    ...
    async def close(self) -> None:
        """Closes the connection to beanstalkd. The client instance
    ...
    async def reserve(self, timeout: Optional[int] = None) -> Job:
        ...
        which will raise a :class:`TimedOutError <greenstalk.TimedOutError>`
        if a job cannot be reserved within that time.
    ...
    async def release(
        ...
        :param priority: An integer between 0 and 4,294,967,295
                         where 0 is the most urgent.
        :param delay: The number of seconds to delay the job for.
        """
    ...
    async def watch(self, tube: str) -> int:
        ...
        """
        return await self._int_cmd(
            b"watch %b" % tube.encode("ascii"), b"WATCHING"
        )

    async def ignore(self, tube: str) -> int:
        """Removes a tube from the watch list. Returns the number of tubes
        this client is watching.

        :param tube: The tube to ignore.
        """
        return await self._int_cmd(
            b"ignore %b" % tube.encode("ascii"), b"WATCHING"
        )
